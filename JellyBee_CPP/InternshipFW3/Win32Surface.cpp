...
LRESULT CALLBACK WndProcedure(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		{
			if (context.OnKey != 0)
			{
				context.OnKey((int) wParam, false);
			}
		}
		break;
	case WM_LBUTTONDOWN:		
	case WM_RBUTTONDOWN:		
		mouseDown = true;		
		{
			if (context.OnMouse != 0)
			{
				context.OnMouse(0, x, y);
			}
		}
		break;
...
bool InitWindow(int width, int height)
{
	...
	hWnd = CreateWindow( "OpenGLES2.0", "B4+1 Studio", WS_VISIBLE | WS_SYSMENU,
